...
- [ ] <!--e7-t06--> Frontend API client and Saved jobs page: `src/api/jobs.ts` with types, the paste form, the list with status filter, the empty state, and navigation between the assistant and saved jobs at the top of the sidebar (left out of `e5-t11` while there was one destination) · AC-E7.10 · repo: FE · learn: typing an API response once and reusing it; optimistic vs confirmed list updates
